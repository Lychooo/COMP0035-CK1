...
def upsert_dim_tables(conn: sqlite3.Connection, df: pd.DataFrame) -> None:
    ...
    cur.executemany(
        "INSERT OR IGNORE INTO programme(university_id, name) VALUES (?, ?)",
        prows
    )
    conn.commit()
...
def main():
    ...
    try:
        ...
        print(f"   - Tables -> university: {n_uni}, programme: {n_prog}, survey_year: {n_year}, survey_result: {n_res}")
        print(f"   - View  -> v_result (ready for queries)")
    finally:
        conn.close()
